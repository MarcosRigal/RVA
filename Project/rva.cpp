...
// TASK 2
static Ptr<Feature2D> createDetector(const std::string &detector)
{
    std::string d = detector; std::transform(d.begin(), d.end(), d.begin(), ::tolower);
    if(d=="sift")
#if CV_VERSION_MAJOR >= 4
        return SIFT::create();
#else
        return xfeatures2d::SIFT::create();
#endif
    else if(d=="akaze")
        return AKAZE::create();
    else if(d=="brisk")
        ...
        return ORB::create(2000);
}

void rva_calculaKPsDesc(const Mat &img, vector<KeyPoint> &kps, Mat &desc, const std::string &detector)
{
    Ptr<Feature2D> det = createDetector(detector);
...
{
    if(matches.size()<4){ H.release(); return; }
    vector<Point2f> p1,p2; p1.reserve(matches.size()); p2.reserve(matches.size());
...
{
    good.clear(); if(matches.size()<4) return;
    vector<Point2f> pm,ps;
    for(auto &m:matches){ pm.push_back(kpsM[m.queryIdx].pt); ps.push_back(kpsS[m.trainIdx].pt);}    
    Mat mask; findHomography(pm,ps,RANSAC,3.0,mask);
    for(size_t i=0;i<matches.size();++i) if(mask.at<uchar>(int(i))) good.push_back(matches[i]);
}

void rva_dibujaPatch(const Mat &scene, const Mat &patch, const Mat &H, Mat &output)
{
    if(H.empty()){ scene.copyTo(output); return; }
    scene.copyTo(output);
    Mat warped; warpPerspective(patch,warped,H,scene.size());
    Mat mask; cvtColor(warped,mask,COLOR_BGR2GRAY); threshold(mask,mask,1,255,THRESH_BINARY);
    warped.copyTo(output,mask);
}

void rva_mostrarInfo(Mat &img, const vector<String> &info, const vector<Point2f> &verts)
{
    if(info.empty()||verts.size()!=4) return;
    Point org = verts[0] + Point2f(10,20);
    int h=25; for(size_t i=0;i<info.size();++i)
